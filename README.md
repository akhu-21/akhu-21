# 👋 Hi, I'm Akhil Kumar

**Aspiring AI/ML Engineer | Data Science Enthusiast | Full-Stack Developer**

---

## 💫 About Me

* 🔭 Currently working on **Data Science & Web Development projects**
* 🌱 Learning **Machine Learning, Artificial Intelligence, and Full Stack Development**
* 💡 Passionate about solving **real-world problems using data-driven solutions**
* 🎯 Goal: To become a **Data Scientist / AI Engineer**

---

## 🛠️ Skills & Technologies

### 💻 Programming

* Python
* SQL
* C++

### 📊 Data Science & Machine Learning

* Pandas
* NumPy
* Matplotlib
* Seaborn
* Scikit-learn

### 🌐 Web Development

* Django
* HTML, CSS, Bootstrap

### ⚙️ Tools

* Git & GitHub
* VS Code
* Jupyter Notebook

---

## 📌 Projects

### 🎓 Student Result Management System

* Developed a system to **automate academic record management**
* Enabled faculty to **manage results and generate reports efficiently**
* Provided students with **secure online access to results**
* Improved **accuracy, security, and efficiency** by reducing manual work
* **Tech Stack:** Python/PHP, MySQL, HTML, CSS, JavaScript

---

### 🧬 Genetic Risk Predictor for Lifestyle Diseases

* Built a **Machine Learning model** to predict risks of diseases (e.g., diabetes, hypertension)
* Performed **data preprocessing and feature selection**
* Achieved **85–90% prediction accuracy**
* **Tech Stack:** Python, Django, Scikit-learn, Pandas, NumPy

---

### 🤖 AI-Driven Career Path Recommendation System

* Developed a **full-stack AI web application** for personalized career recommendations
* Analyzed **skills, interests, and academic data**
* Implemented **Random Forest algorithm** for predictions
* Achieved **88% accuracy**
* Provided **structured career roadmaps with explanations**
* **Tech Stack:** Django, HTML, CSS, JavaScript, Machine Learning

---

## 📈 GitHub Stats

![](https://github-readme-stats.vercel.app/api?username=Akhu-21\&theme=dark\&hide_border=false\&include_all_commits=false\&count_private=false)
![](https://nirzak-streak-stats.vercel.app/?user=Akhu-21\&theme=dark\&hide_border=false)

![](https://github-readme-stats.vercel.app/api/top-langs/?username=Akhu-21\&theme=dark\&hide_border=false\&layout=compact)

---

## 🌐 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/www.linkedin.com/in/m-p-akhil-kumar)
[![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:akhilkumarmp2003@gmail.com) 
---

## ⚡ Fun Fact

* 💡 Passionate about building **intelligent systems that solve real-world problems**
* 💡 Enjoy combining **Machine Learning with Web Development** to create impactful applications
